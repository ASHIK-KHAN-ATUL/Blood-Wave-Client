...
const BloodRequest = () => {
  ...
  const {
    ...
    queryKey: ["blood-request", user?.email],
    enabled: !!user?.email,
    queryFn: async () => {
      const res = await axiosSecure.get(
        `/blood-requests/donor?email=${user?.email}`
      );
      return res.data;
    },
  });
  ...
  const handleStatusChange = async (id, status) => {
    ...
    const res = await axiosSecure.patch(`/blood-requests/${id}`, {
      status,
      donorId,
    });
  ...
  return (
    <div className="p-4 text-black font-semibold">
      <div className="bg-gradient-to-r from-pink-200 via-purple-200 to-blue-200 rounded-2xl shadow-lg p-6 text-center border border-purple-300 mb-6">
        ...
        <p className="mt-2 text-gray-700">Stay strong and help save lives ❤️</p>
      </div>
        ...
        >
          ...
          >
            ...
            <div className="grid grid-cols-2 gap-x-6 gap-y-3 text-gray-800 mb-6">
              <p>
                <strong>Name:</strong> {selectedRequest.sendername || "N/A"}
              </p>
              <p>
                <strong>Phone:</strong> {selectedRequest.senderphone || "N/A"}
              </p>

              <p>
                <strong>Email:</strong> {selectedRequest.senderemail || "N/A"}
              </p>
              <p>
              ...
              <p>
                ...
                {selectedRequest.emergencyType || "N/A"}
              </p>
              <p>
              ...
              <p>
                ...
                {selectedRequest.needDate
                  ? new Date(selectedRequest.needDate).toLocaleDateString()
                  : "N/A"}
              </p>
            </div>
            ...
            <div className="flex justify-center gap-6">
              <button
                ...
                className={`px-6 py-2 rounded-xl font-semibold text-white shadow-md transition-colors duration-200 ${
                  ...
                  mainUser.availability === "unavailable"
